...
import { websiteFormValidator } from '../util';
...
const CreateWebsiteForm = () => {
  ...
  const websiteCategories = useSelector(
    (state) => state.metadata?.data?.websiteCategories
  );
  const [isConfirmationDialogOpen, setIsConfirmationDialogOpen] =
    useState(false);
  const formData = useSelector((state) => state.websites?.createWebsiteForm?.formData);
  const isFormLoading = useSelector((state) => state.websites?.createWebsiteForm?.isLoading);
  
  const { error, validateField, validateForm, clearFormError } =
    useFormValidator(websiteFormValidator);
  ...
  const setFormData = (data) => {
    dispatch(setCreateWebsiteFormData(data));
  };
  ...
  const handleCloseModal = () => {
    ...
    dispatch(createWebsiteCancelled()); // if creating website
    clearFormError();
  };

  const handleSelectFiles = (field, files) => {
    if (field === 'attachmentImages') {
  ...
  const handleCreateClick = () => {
    ...
    const data = {
      projectType: PROJECT_TYPE.WEBSITES,
      name,
      description,
      categories,
      sourceCode,
    ...
    <>
      ...
      >
        <div className={styles.modal__content}>
          ...
          >
            <Dropdown
              options={generateOptions(websiteCategories)}
          ...
          <FormInputWrapper title='Add Website Icon*' error={error.attachmentIcon}>
            <FileInput
